...
mod2a = summary(
  feols(arr_delay ~ dep_tod / dep_delay | 
          month + year + day_of_week + origin_airport_id + dest_airport_id,          
        data = air), 
  vcov = ~month + origin_airport_id
  )
mod3a = summary(
  feols(arr_delay ~ dep_tod / dep_delay | 
          month + year + day_of_week + origin_airport_id + dest_airport_id,          
        data = air),
  vcov = ~month + origin_airport_id + tail_num
  )
...
```{r reghdfe}
#| include: FALSE
time_reghdfe = fread('~/air-reghdfe.csv')
...
[^3]: Note that I'm going to use a `dep_tod / dep_delay` expansion on the RHS to get the [full marginal effect](../interaction-effects) of the interaction terms. Don't worry too much about this if you haven't seen it before (click on the previous link if you want to learn more).

...
Does that sound familiar? I believe it should, because something like that has happened to me in every single empirical project I have ever worked on. I end up estimating multiple versions of the same underlying regression model &mdash; even putting them side-by-side in a regression table, where the only difference across columns is slight tweaks to the way that the SEs were calculated.
